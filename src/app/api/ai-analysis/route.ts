...
export async function POST(req: NextRequest) {
  try {
    ...
    // Call Yahoo Finance directly to avoid Vercel serverless double-concurrency loop freezes
    const [quote, summaryData, fundamentalsA] = await Promise.all([
    ...
    if (!quote) {
      return NextResponse.json({ error: "فشل جلب بيانات السهم الحالية من Yahoo Finance." }, { status: 502 });
    }

    // Process quote variables
    const pe = quote.trailingPE ?? null;
    const eps = quote.epsTrailingTwelveMonths ?? quote.epsForward ?? null;
    const divYield = quote.trailingAnnualDividendYield ? (quote.trailingAnnualDividendYield * 100).toFixed(2) : null;
...
البيانات المالية التفصيلية المتاحة:
${summaryData ? JSON.stringify(summaryData).substring(0, 3000) : "غير متوفرة حالياً"}.
...
4. **التوصية والتقييم النهائي**: حدد بوضوح التوصية (شراء قوي، شراء، احتفاظ، تجنب/بيع) مع سرد 3 نقاط أساسية تدعم التوصية.`;
    ...
    if (fundamentalsA && Array.isArray(fundamentalsA) && fundamentalsA.length > 0) {
      const balanceSheet = fundamentalsA[0];
      if (balanceSheet) {
        const totalLiab = balanceSheet.totalLiabilitiesNetMinorityInterest ?? balanceSheet.pretaxIncome ?? 0;
        const totalEquity = balanceSheet.stockholdersEquity ?? 0;
        const currentAssets = balanceSheet.currentAssets ?? 0;
        const currentLiab = balanceSheet.currentLiabilities ?? 0;
...
3. توافق عائد التوزيع ومكرر الربحية يقدم **${pe && pe < 20 ? "نقطة دخول مناسبة للمستثمر طويل الأجل" : "فرصة للاحتفاظ بالسهم حالياً وانتظار تقارير الأرباح الربعية القادمة"}**.`;
